...
def main():
    ...
    regression_model = ODEModel()
    regression_model.to(device)

    optimizer = Adam(regression_model.parameters(), lr=1e-3)
    ...
    for epoch in range(1, epochs + 1):
        ...
        y_train = torch.flatten(y_train[:-1, :, :], start_dim=0, end_dim=1)
        y_dot_train = torch.flatten(y_dot_train, start_dim=0, end_dim=1)

        optimizer.zero_grad()
        y_dot_pred = regression_model(None, y_train)
        L_train = regression_loss_function(y_dot_pred, y_dot_train)
        L_train.backward()
        optimizer.step()

        with torch.no_grad():
            y0_test = y_test[0, :, :]
            y_test_pred = odeint(regression_model, y0_test, t_test, method="rk4")
            L_test = test_loss_function(y_test_pred, y_test)

            line_integral = torch.zeros(batch_size_test).to(device)

            F = regression_model(None, y_test[0, :, :])
            ...
            for i in range(1, y_test.shape[0] - 1):
                F = regression_model(None, y_test[i, :, :])
                F = nn.functional.normalize(F, dim=1)
                dr = y_dot_test[i, :, :]
                dr = nn.functional.normalize(dr, dim=1)
    ...
    np.savetxt("regression_loss.txt", losses)
